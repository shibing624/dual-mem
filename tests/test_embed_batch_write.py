"""Post-extract embed batching and optional L1+Gate pre-merge."""
...
async def test_embed_merge_l1_gate_one_batch(tmp_storage):
    embed = CountingEmbed()
    factory = ComponentFactory(
        settings=Settings(
            mode="system1",
    ...
    assert embed.queued_calls == []
    assert embed.batch_calls[0] == [dialogue, *user_queries]
    # pre-merge batch + post-extract batch
    assert len(embed.batch_calls) == 2


async def test_embed_merge_l1_gate_off_uses_queue(tmp_storage):
    embed = CountingEmbed()
    factory = ComponentFactory(
        settings=Settings(
            mode="system1",
...
async def test_reconcile_sync_basic_info_only_writes_l0_no_enqueue(tmp_storage):
    ...
    factory = ComponentFactory(
        settings=Settings(
            ...
            gate_enabled=False, reconcile_sync=True,
        ),
        embed=embed,
    ...
    stored_ids, _gate, _eph = await agent.run(
        raw_node=raw, content=raw.content, embedding=raw.embedding,
